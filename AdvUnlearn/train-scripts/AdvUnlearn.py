...
def AdvUnlearn(prompt, dataset_retain, retain_batch, retain_train, retain_step, retain_loss_w , attack_method, train_method, norm_layer, component, start_guidance, negative_guidance, iterations, save_interval, lr, config_path, ckpt_path, diffusers_config_path, output_dir, devices, seperator=None, image_size=512, ddim_steps=50, adv_prompt_num=3, attack_embd_type='word_embd', attack_type='prefix_k', attack_init='latest', warmup_iter=200, attack_step=30, attack_lr=1e-2, adv_prompt_update_step=20):
    ...
    if seperator is not None: # 여러 프롬프트를 한 번에 학습하는 경우 (--prompt "nudity, sexual, violence" --seperator ",")
    ...
    model_orig, sampler_orig, model, sampler = get_models(config_path, ckpt_path, devices) # model_orig는 학습x reference 모델
    model_orig.eval()

    # Setup tainable model parameters
    if 'text_encoder' in train_method: # 텍스트 인코더만 학습하는 경우
    ...
    for i in pbar:
        ...
        if i % adv_prompt_update_step == 0: # 1 iteration마다 새로운 adversarial prompt 만듦
            
            # retain prompt를 한 번씩은 샘플링할 수 있도록 아직 한 번도 샘플링되지 않은 프롬프트 개수 확인     
            if retain_dataset.check_unseen_prompt_count() < retain_batch:
                retain_dataset.reset()

            # 현재 unlearn 대상 word 하나를 랜덤으로 샘플링
            ...
            # get diffusion conditional embedding for the prompt 
            ## diffusion 모델 내부 latent embedding
            emb_0 = model_orig.get_learned_conditioning(['']) # uncoditional embedding (빈 문자열 기준)
            emb_p = model_orig.get_learned_conditioning([word]) # conditional (실제 prompt 기반)
            
            # ===== ** Attack ** : 적대적 프롬프트 생성
            if i >= warmup_iter: # warmup(초반) 동안은 adversarial attack을 하지 않음.
                custom_text_encoder.text_encoder.eval()
                custom_text_encoder.text_encoder.requires_grad_(False)
                model.eval()
                if attack_round == 0: # adv_word_embd없이 시작
                    if attack_embd_type == 'word_embd': # 실제 입력 embdding을 조작
                    ...
                    elif attack_embd_type == 'condition_embd': # diffusion 내부에서 사용하는 condition embedding을 자체 조작
        ...
        if 'text_encoder' in train_method:
            ...
            custom_text_encoder.text_encoder.requires_grad_(False)
            model.train()
        opt.zero_grad() # 이전 step에서 누적된 gradient 초기화
        ...
        if retain_train == 'reg':
            ...
            retain_emb_p = model_orig.get_learned_conditioning(retain_words) # frozen 모델 기준
            ...
            retain_emb_n = custom_text_encoder(input_ids = retain_input_ids, inputs_embeds=retain_text_embeddings)[0] # 현재 모델 기준
        else:
        ...
        if i < warmup_iter: # warmup 시에는 원래 prompt로만 학습, 초기 단계에서 모델이 너무 심하게 바뀌는 걸 막음
        ...
        if retain_train == 'iter': # 모델이 unlearn에 의해 retain prompt를 잊지 않도록 도와주는 디버깅 루프
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iterations', help='iterations used to train', type=int, required=False, default=100)
    parser.add_argument('--save_interval', help='iterations used to train', type=int, required=False, default=20)
    parser.add_argument('--lr', help='learning rate used to train', type=int, required=False, default=1e-5)
    
    # Attack hyperparameters
    parser.add_argument('--adv_prompt_num', help='number of prompt token for adversarial soft prompt learning', type=int, required=False, default=3) # token 수
    ...
    parser.add_argument('--attack_type', help='the attack type: append or add', type=str, required=False, default='suffix_k', choices=['replace_k' ,'add', 'prefix_k', 'suffix_k', 'mid_k', 'insert_k', 'per_k_words'])
    ...
    parser.add_argument('--attack_step', help='adversarial attack steps', type=int, required=False, default=5) # attack step 설정
    ...
    parser.add_argument('--warmup_iter', help='the number of warmup interations before attack', type=int, required=False, default=20)
