...
def get_train_loss(model, model_orig, text_encoder, sampler, emb_0, emb_p, emb_n, start_guidance, negative_guidance, devices, ddim_steps, ddim_eta, image_size, criteria, adv_input_ids, attack_embd_type, adv_embd=None):
    ...
    t_enc = torch.randint(ddim_steps, (1,), device=devices[0]) # 전체 50 DDIM step중 랜덤하게 선택된 중간 단계까지만 생성해서 그 𝑥_𝑡를 학습에 사용
...
# CA 구조, retain 기준 across alignment (e_n vs retain_e_p) ; A를 지우고 B처럼 보이게 만들어줘.
...
# retain 전용 학습, unlearn + retain loss
