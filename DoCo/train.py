...
def main(args):
    ...
    if args.multi_ckpt_path != "":
        # print('load history state')
        _, inner_dict = torch.load(args.multi_ckpt_path).popitem()
        unet.load_state_dict(inner_dict, strict=False)

    shadow_unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="unet", revision=args.revision
    )
    ...
    discriminator, criterion, optimizer_D = init_discriminator(lr = args.dlr)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                encoder_anchor_hidden_states = text_encoder(batch["input_anchor_ids"])[0]

                # Predict the noise residual
                ### target개념 latent 예측 (학습 대상)
                model_pred = unet(
                    noisy_latents, timesteps, encoder_hidden_states
                ).sample
                ### anchor개념 latent 예측 (비교용) 
                with torch.no_grad():
                ...
                if args.loss_type_reverse == "model-based":
                    ...
                    # Chunk the noise and model_pred into two parts and compute the loss oxn each part separately.
                ...
                if args.with_prior_preservation:
                    ...
                    ### latent로부터 복원된 이미지 -> discriminator 또는 분포 비교에 사용되는 x_0
