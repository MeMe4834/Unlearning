...
class CLIPScoreEvaluator:
    ...
    def __call__(self, pil_image, prompt):
        ...
        return F.cosine_similarity(image_embeds, text_embeds).item()
...
def main():
    # output save paths
    clip_text_model = CLIPTextModel.from_pretrained("openai/clip-vit-base-patch32").to(args.device)
    ...
    eval_func = Eval(args)
    clip_eval = CLIPScoreEvaluator(clip_model, clip_processor, args.device)
    ...
    result_path = os.path.join(args.save_dir, "result_i2p.csv")
    ...
    for _iter, data in dataset.iterrows():
        ...
        if 'artists-' in args.category:
            ...
            clip_score = clip_eval(imgs[0], target_prompt)
